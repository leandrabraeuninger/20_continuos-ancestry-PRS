...
def simulate_admixture_proportions(cfg: SimConfig, rng=None, population_weights=None,
                                    n_individuals=None):
    """
    Per-individual true ancestry proportions, (N, K), rows sum to 1.

    population_weights: optional (K,) array of relative weights across
        populations. If provided, the Dirichlet concentration vector becomes
        `admixture_alpha * K * weights / weights.sum()` instead of a flat
        `admixture_alpha` for every population. This shifts where the mass
        of the simplex sits -- e.g. weights=[8,1,1,1,1] makes most
        individuals' plurality ancestry population 0 (e.g. "European"),
        while still allowing admixture controlled by `admixture_alpha`.
        If None, behaves exactly as before (symmetric Dirichlet).

    n_individuals: override cfg.n_individuals, e.g. for generating a
        differently-sized split.
    """
    if rng is None:
        rng = np.random.default_rng(cfg.seed + 1)
    K = cfg.n_populations
    n = n_individuals if n_individuals is not None else cfg.n_individuals

    if population_weights is None:
        alpha = np.full(K, cfg.admixture_alpha)
    else:
        w = np.asarray(population_weights, dtype=float)
        assert len(w) == K, f"population_weights must have length {K}"
        w = w / w.sum()
        # Scale so total concentration mass matches the symmetric case
        # (admixture_alpha * K), just redistributed unevenly across
        # populations. This keeps "how admixed is a typical individual"
        # comparable across different weight vectors -- only WHICH
        # population dominates changes, not how mixed people are overall.
        alpha = cfg.admixture_alpha * K * w

    q = rng.dirichlet(alpha, size=n)
    return q


def simulate_genotypes(cfg: SimConfig, p_pop: np.ndarray, q: np.ndarray, seed_offset: int = 2):
    """
    ...
    seed_offset: lets callers generating multiple independent splits use
        distinct RNG streams (e.g. seed_offset=2 for split A, =1002 for
        split B) while still reusing the same cfg.seed base, so splits
        don't share correlated randomness.
